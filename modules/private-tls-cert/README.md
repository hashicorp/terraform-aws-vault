...
This module can be used to generate the public and private keys of a private TLS certificate, along with the
public and private keys of the Certificate Authority (CA) used to sign the TLS certificate. 

These certificates are meant to be used with **private** services, such as a Vault cluster accessed solely within your 
AWS account. For publicly-accessible services, especially services you access through a web browser, you should NOT use 
this module, and instead get certificates from a commercial CA, such as [Let's Encrypt](https://letsencrypt.org/).
...
1. Run `terraform apply`. The output will show you the paths to the generate cert files:

    ```
    Outputs:
    
    ca_cert_private_key_file_path = ca.crt.key
    ca_cert_public_key_file_path = ca.crt.pem
    cert_private_key_file_path = vault.key.pem
    cert_public_key_file_path = vault.crt.pem
    ```
...
Distribute the certificate private and public keys (the files at `cert_private_key_file_path` and 
`cert_public_key_file_path`) to the services that will use them to handle TLS connections (e.g. Vault). For example,
to run Vault with the [run-vault module](/modules/run-vault), you need to pass it the TLS certs: 
...
Unless you plan on using this CA certificate to sign future certificates, we **strongly** recommend deleting the CA 
private key (the file at `ca_cert_private_key_file_path`). This will make it impossible for someone to steal this 
private key and use it to sign their own fake TLS cert. If, in the future, you ever need another TLS cert, you can 
generate a completely new CA cert using this module.

If you decide to keep the CA private key, then you should encrypt it (e.g. using [KMS](https://aws.amazon.com/kms/)) 
and store it in a secure, trusted location.
...
1. **Finally, consider throwing away the CA private key.**
    ...
    - Contrast this to protecting your CA private key. There are tradeoffs either way so choose the option that makes
